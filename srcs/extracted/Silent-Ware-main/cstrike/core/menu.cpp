...
void MENU::RenderMainWindow()
{
	...
	if (flBackgroundAlpha > 0.f)
	{
		if (C_GET(unsigned int, Vars.bMenuAdditional) & MENU_ADDITION_DIM_BACKGROUND)
			D::AddDrawListRect(ImGui::GetBackgroundDrawList(), ImVec2(0, 0), vecScreenSize,
			C_GET(ColorPickerVar_t, Vars.colPrimtv1).colValue.Set<COLOR_A>(125 * flBackgroundAlpha), DRAW_RECT_FILLED);
...
if (settings_window)
	{
		/*C_GET(unsigned int, Vars.bMenuAdditional) |= (1 << 0) | (1 << 1); */
		...
		ImGui::Begin(CS_XOR("Settings"), &settings_window, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse);
		{
			...
			ImGui::BeginChildSection(CS_XOR("Misc Settings"), ImVec2(0, ImGui::GetContentRegionAvail().y), true);
			{
				...
				/*ImGui::HotKey(CS_XOR("Panic Key"), &C_GET(unsigned int, Vars.nPanicKey));
				if (ImGui::Button(CS_XOR("Unload Cheat")))
				{
				...
				ImGui::SeparatorText(CS_XOR("Color palette"));
	...
	ImGui::Begin(CS_XOR("Xenon"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBackground);
	{
		...
		//pWindowDrawList->AddText(FONT::pCheatLogo, 35.f * flDpiScale, ImVec2(70.000f * flDpiScale + p.x, 15.000f * flDpiScale + p.y), ImColor(0.60f, 0.60f, 0.60f, 0.70f), "SilentWare");

		pWindowDrawList->AddText(FONT::pCheatLogo, 35.f * flDpiScale, ImVec2(20.000f * flDpiScale + p.x, 15.000f * flDpiScale + p.y), ImColor(0.60f, 0.60f, 0.60f, 0.70f), "SilentWare");
		//pWindowDrawList->AddText(FONT::pCheatLogo, 35.f * flDpiScale, ImVec2(20.000f * flDpiScale + p.x, 15.000f * flDpiScale + p.y), ImColor(0.60f, 0.60f, 0.60f, 0.70f), "SEXWARE");
		...
		if (ImGui::TabButton("N", "RageBot", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 0)
		{
			nCurrentMainTab = 0;
			active = true;
		}

		if (ImGui::TabButton("Q", "AntiAim", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 1)
		{
			nCurrentMainTab = 1;
			active = true;
		}

		if (ImGui::TabButton("P", "LegitBot", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 2)
		{
			nCurrentMainTab = 2;
			active = true;
		}
		...
		if (ImGui::TabButton("I", "Visuals", ImVec2(190 * flDpiScale, 70)) && nCurrentMainTab != 3)
		{
			nCurrentMainTab = 3;
			active = true;
		}

		if (ImGui::TabButton("O", "Misc", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 4)
		{
			nCurrentMainTab = 4;
			active = true;
		}
		...
		if (ImGui::TabButton("S", "Config", ImVec2(190, 70)) && nCurrentMainTab != 8)
		{
			nCurrentMainTab = 8;
			active = true;
		}
...
auto Temp() -> C_CSPlayerPawn*
{
	...
	SchemaClassInfoData_t* pClassInfo2 = nullptr;
	LocalPlayerPawn->GetSchemaClassInfo(&pClassInfo2);
	if (pClassInfo2 == nullptr)
		return LocalPlayerPawn;

	const FNV1A_t uHashedName2 = FNV1A::Hash(pClassInfo2->szName);
	if (uHashedName2 != FNV1A::HashConst("C_CSPlayerPawn"))
		return LocalPlayerPawn;
	...
	auto pPlayerPawn = I::GameResourceService->pGameEntitySystem->Get<C_CSPlayerPawn>(hObserverTarget.GetHandleValue());
	if (!pPlayerPawn)
		return nullptr;
...
void MENU::RenderSpectators()
{
	if (C_GET(bool, Vars.bSpectatorList))
	{
		...
		C_CSPlayerPawn* pLocalPawn = Temp();
		if (!pLocalPawn)
			return;

		ImGui::SetNextWindowBgAlpha(0.f);
		ImGui::SetNextWindowSize(ImVec2(165, 150));
		ImGui::Begin("Spectators", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize);
		{
			ImGuiStyle& style = ImGui::GetStyle();
			style.WindowBorderSize = 0.0f;
			auto background_list = ImGui::GetBackgroundDrawList();
			...
			// Ðèñóåì òåêñò, ñäâèíóâ åãî åùå íà 2 ïèêñåëÿ âïðàâî
			draw_list->AddText(FONT::pMain, 17.f, ImVec2(xy.x + 43, xy.y + 14), ImColor(255, 255, 255, 255), "Spectators"); // Name
			...
			for (auto& entity : entity_list)
			{
				...
				C_CSPlayerPawn* pawn = pPlayer->GetPlayerPawn();
				if (!pawn)
					continue;
				...
				SchemaClassInfoData_t* pClassInfo2 = nullptr;
				pawn->GetSchemaClassInfo(&pClassInfo2);
				if (pClassInfo2 == nullptr)
					continue;
				...
				auto obs_pawn = pPlayer->GetObserverPawn();
				if (!obs_pawn)
					continue;

				SchemaClassInfoData_t* pClassInfo3 = nullptr;
				obs_pawn->GetSchemaClassInfo(&pClassInfo3);
				if (pClassInfo3 == nullptr)
					continue;

				const FNV1A_t uHashedName3 = FNV1A::Hash(pClassInfo3->szName);
				if (uHashedName3 != FNV1A::HashConst("C_CSObserverPawn"))
					continue;
				...
				//L_PRINT(LOG_INFO) << obs_service->GetObserverTarget().GetEntryIndex();
				//L_PRINT(LOG_INFO) << obs_service->GetObserverTarget().ToInt();
				//L_PRINT(LOG_INFO) << "";
				...
				if (obs_service->GetObserverTarget().GetHandleValue() != handle_local)
					continue;
...
void MENU::UpdateStyle(ImGuiStyle* pStyle)
{
	...
	style.Colors[ImGuiCol_Tab] = C_GET(ColorPickerVar_t, Vars.colPrimtv1).colValue.GetVec4(0.80f);
	...
	style.Colors[ImGuiCol_TabUnfocused] = C_GET(ColorPickerVar_t, Vars.colAccent1).colValue.GetVec4(0.70f);
...
//		ImGui::SliderFloat(CS_XOR("Aim range"), &C_GET(float, Vars.iAimRange), 1.f, 135.f);
//		ImGui::SliderFloat(CS_XOR("Smoothing"), &C_GET(float, Vars.flSmoothing), 1.f, 100.f, "%.2f");
//		ImGui::Checkbox(CS_XOR("Trigger"), &C_GET(bool, Vars.trigger));
//		//ImGui::Checkbox(CS_XOR("Aim always on"), &C_GET(bool, Vars.bLegitbotAlwaysOn));
//	}
//	ImGui::EndChildSection();
//
...
void T::LegitBot()
{
	ImGui::BeginChildSection(CS_XOR("Aimbot"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		float xPos = (windowSize.x - textSize.x) * 0.5f;
		float yPos = (windowSize.y - textSize.y) * 0.5f;

		ImGui::SetCursorPos(ImVec2(xPos, yPos));
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 0.5f), "Coming soon...");
	}
	ImGui::EndChildSection();
}


void T::RageBot() // bug const char weapons XD
{

	float startX = 203 * MENU::flDpiScale;
	float startY = 45 * MENU::flDpiScale;
	...
	{
		if (ImGui::Selectable(weapons[i], currentWeapon == i, 0, ImVec2(60 * MENU::flDpiScale, 24 * MENU::flDpiScale)))
...
void T::VisualsPlayers()
{
	...
	ImGui::BeginChildSection(CS_XOR("Player Esp"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		ImGui::BeginDisabled(!C_GET(bool, Vars.bVisualOverlay));
		{
			...
			ImGui::ToggleButton(CS_XOR("Weapon Name"), &C_GET(TextOverlayVar_t, Vars.overlayWeaponName).bEnable);
			ImGui::ToggleButton(CS_XOR("Weapon Icon"), &C_GET(TextOverlayVar_t, Vars.overlayWeaponIcons).bEnable);
			...
			ImGui::MultiCombo(CS_XOR("Flags"), &C_GET(unsigned int, Vars.pEspFlags), arrEspFlags, CS_ARRAYSIZE(arrEspFlags));
			ImGui::ToggleButton(CS_XOR("Backtrack Render"), &C_GET(bool, Vars.bBackTrackRender));
		}
		ImGui::EndDisabled();
	}
	ImGui::EndChildSection();
...
void T::VisualsView()
{
	ImGui::BeginChildSection(CS_XOR("Visuals Camera"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		ImGui::ToggleButton(CS_XOR("Remove Legs"), &C_GET(bool, Vars.bRemoveLegs));
		ImGui::ToggleButton(CS_XOR("Aspect Ratio"), &C_GET(bool, Vars.bAspectRatio));
		ImGui::SliderFloat(CS_XOR("Aspect Ratio Value"), &C_GET(float, Vars.flAspectRatio), 0.3f, 3.f);
	...
	ImGui::BeginChildSection(CS_XOR("Modelchanger"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{

		ImGui::ToggleButton(CS_XOR("Enable##player_model"), &C_GET(bool, Vars.bPlayerModelChanger));

		if (ImGui::BeginListBox(CS_XOR("##player_model.list"), ModelChanger->vecPlayerModels.size(), 5))
		{
...
void T::Miscellaneous()
{
	ImGui::BeginChildSection(CS_XOR("Misc General"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		ImGui::ToggleButton(CS_XOR("Enemy Bullet Trace"), &C_GET(bool, Vars.bBulletTraceEnemy));
		ImGui::ColorEdit4(CS_XOR("Enemy Bullet Trace Color"), &C_GET(Color_t, Vars.colBulletTraceEnemy));
		ImGui::ToggleButton(CS_XOR("Local Bullet Trace"), &C_GET(bool, Vars.bBulletTraceLocal));
		ImGui::ColorEdit4(CS_XOR("Local Bullet Trace Color"), &C_GET(Color_t, Vars.colBulletTraceLocal));
...
//void T::MiscellaneousConfigs()
//{
//    //Do not delete! This will make it impossible to build release x64
//}
//
//void T::MiscellaneousMain()
//{
//	//Do not delete! This will make it impossible to build release x64
//}
