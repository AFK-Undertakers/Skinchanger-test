...
bool ImGui::TabButtonEx(const char* ico, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	bb.Min + ImVec2(0.000f, 0.000f),
	bb.Max + ImVec2(bb.Min.x - bb.Min.x - 20, bb.Min.y - bb.Min.y),
	ImColor(0.200f, 0.200f, it_nameanim->second, 0.100f),
	ImColor(0.00f, 0.00f, 0.00f, 0.000f),
	ImColor(0.00f, 0.00f, 0.00f, 0.000f),
	ImColor(0.200f, 0.200f, it_nameanim->second, 0.100f));
	...
	float accent_color[4] = {
		ImGui::GetStyleColorVec4(ImGuiCol_TitleBg).x,
		ImGui::GetStyleColorVec4(ImGuiCol_TitleBg).y,
		ImGui::GetStyleColorVec4(ImGuiCol_TitleBg).z,
		ImGui::GetStyleColorVec4(ImGuiCol_TitleBg).w
	};

	// Active tab highlight
	if (MENU::nCurrentMainTab == window->GetID(label))
	{
		window->DrawList->AddRectFilledMultiColor(
		ImVec2(0.000f + pos.x, 0.000f + pos.y),
		ImVec2(gradient_line + pos.x, 40 + pos.y),
		ImColor(accent_color[0], accent_color[1], accent_color[2], 0.100f),
		ImColor(accent_color[0], accent_color[1], accent_color[2], 0.000f),
		ImColor(accent_color[0], accent_color[1], accent_color[2], 0.000f),
		ImColor(accent_color[0], accent_color[1], accent_color[2], 0.100f));
	}
	...
	// Label rendering
	RenderTextClipped(
	bb.Min + style.FramePadding + ImVec2(65, 0),
	bb.Max - style.FramePadding + ImVec2(60, 0),
	...
	GetWindowDrawList(), vtx_idx_1, vtx_idx_2,
	bb.Min + ImVec2(20, 0),
	bb.Max + ImVec2(bb.Min.x - bb.Min.x - 20, 20),
	ImColor(
	std::max(0.4f, it_nameanim->second * accent_color[0]),
	std::max(0.4f, it_nameanim->second * accent_color[1]),
	std::max(0.4f, it_nameanim->second * accent_color[2]),
	1.00f),
	ImColor(0.05f, 0.05f, 0.05f, 1.00f));
...
void ImGui::ToggleButton(const char* str_id, bool* v)
{
	...
	// Рисуем тень
	ImU32 shadow_col = IM_COL32(0, 0, 0, 80); // Цвет тени
	draw_list->AddRectFilled(frame_bb.Min + ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), shadow_col, height * 0.5f); // Тень
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	//RenderFrame(frame_bb.Min, ImVec2(grab_bb.Max.x, frame_bb.Min.y + 3), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), true, style.GrabRounding); // Ôðåéì ëèëîâîãî öâåòà (çàïîëíåííûé)
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Max.x, frame_bb.Min.y + 1.5), h * 0.35f, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 512); // Êðóæî÷åê :)
